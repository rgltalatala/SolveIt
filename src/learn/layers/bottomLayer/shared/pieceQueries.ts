...
function readSticker(state: CubeState, position: CubiePosition, face: Face): Color {
  const index = faceStickerIndex(face, position)
  return state[face][index]
}

function edgeCubiePositions(): CubiePosition[] {
  return cubieDefinitions.filter((c) => c.exposedFaces.length === 2).map((c) => c.position)
}

function cornerCubiePositions(): CubiePosition[] {
  return cubieDefinitions.filter((c) => c.exposedFaces.length === 3).map((c) => c.position)
}
...
export function cornerWhiteStickerOnD(state: CubeState, cornerPosition: CubiePosition): boolean {
  return faceForWhiteOnCorner(cornerPosition, state) === 'D'
}

export function findEdgeWithColors(state: CubeState, colorA: Color, colorB: Color): CubiePosition | null {
...
export function faceForWhiteOnEdge(edgePosition: CubiePosition, state: CubeState): Face | null {
  const definition = cubieDefinitions.find(
    (cubie) =>
      cubie.position[0] === edgePosition[0] &&
      cubie.position[1] === edgePosition[1] &&
...
export function isMiddleLayerEdge(edgePosition: CubiePosition): boolean {
  return edgePosition[1] === 0 && edgePosition[0] !== 0 && edgePosition[2] !== 0
}

export function whiteStickerOnD(state: CubeState, edgePosition: CubiePosition): boolean {
  return faceForWhiteOnEdge(edgePosition, state) === 'D'
}

export function whiteStickerOnU(state: CubeState, edgePosition: CubiePosition): boolean {
  return faceForWhiteOnEdge(edgePosition, state) === 'U'
}

/** Non-white sticker matches that face’s center (edge connected to its center). */
export function edgeAlignedToSideCenter(state: CubeState, edgePosition: CubiePosition): Face | null {
  const definition = cubieDefinitions.find(
    (cubie) =>
      cubie.position[0] === edgePosition[0] &&
      cubie.position[1] === edgePosition[1] &&
