...
describe('white cross lesson vs cubing.js random scrambles', () => {
  it(
    'lesson demos eventually complete the white cross on several random WCA scrambles',
    async () => {
      const iterations = 8
      for (let i = 0; i < iterations; i += 1) {
        ...
        expect(res.crossComplete, `cross incomplete iter ${i}, steps=${res.lessonStepsSimulated} alg=${algStr}`).toBe(
          true,
        )
      }
    },
