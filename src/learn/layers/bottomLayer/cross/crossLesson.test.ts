...
import { getLessonExecutionMoves, isBackFaceMove, noneHold } from '../../../studentHold'
...
describe('white cross lesson', () => {
  ...
  it('getWhiteCrossLessonStepAsync matches sync on representative student frames', async () => {
    const frames: CubeState[] = [
      ...
      applyMoves(cubeStateToStudentFrame(createSolvedCubeState()), ['D', 'R', 'R']),
    ]
  ...
  it('prioritizes rotate-bottom (D-layer white misalignment) before other steps including middle-layer side-connect', () => {
    const student = applyMoves(cubeStateToStudentFrame(createSolvedCubeState()), ['D', 'R', 'R'])
    expect(getWhiteCrossLessonStep(student).kind).toBe('rotate-bottom')
  })

  const LESSON_SEARCH_MOVES = ['U', "U'", 'F', "F'", 'R', "R'", 'L', "L'", 'B', "B'", 'D', "D'"] as Move[]
  ...
  it('phased permute: rotate-bottom wins when both D permute and middle-layer candidates exist', () => {
    ...
    function dfs(s: typeof base, path: Move[], depth: number): void {
      ...
      if (depth === 0) {
        ...
        if (hasRotate && hasMiddle && getWhiteCrossLessonStep(s).kind === 'rotate-bottom') {
          example = [...path]
        }
        return
      }
      for (const m of LESSON_SEARCH_MOVES) dfs(applyMove(s, m), [...path, m], depth - 1)
    }
    for (let d = 1; d <= 7 && !example; d++) dfs(base, [], d)
    expect(example, 'expected scramble with both rotate-bottom and middle-layer options').not.toBeNull()
    const student = applyMoves(base, example!)
    expect(collectRotateBottomPermuteCandidates(student).length).toBeGreaterThan(0)
    expect(collectMiddleLayerPermuteCandidates(student).length).toBeGreaterThan(0)
    expect(getWhiteCrossLessonStep(student).kind).toBe('rotate-bottom')
  })
  ...
  it('phased permute: D-layer insert-double before middle when no rotate-bottom tier', () => {
    ...
    function dfs(s: typeof base, path: Move[], depth: number): void {
      ...
      if (depth === 0) {
        ...
        const hasMiddle = collectMiddleLayerPermuteCandidates(s).length > 0
        if (!hasRotate && hasInsert && hasMiddle && getWhiteCrossLessonStep(s).kind === 'insert-double') {
          example = [...path]
        }
        return
      }
    ...
    for (let d = 1; d <= 7 && !example; d++) dfs(base, [], d)
    expect(example, 'expected scramble with D insert and middle but no rotate-bottom').not.toBeNull()
    const student = applyMoves(base, example!)
  ...
  it('middle-layer cross edge hugging its partner center: side-connect with exactly one quarter turn', () => {
    ...
    function dfs(s: typeof base, path: Move[], depth: number): void {
      if (example) return
      if (depth === 0) {
        if (getWhiteCrossLessonStep(s).kind === 'side-connect') example = [...path]
        return
      }
      for (const m of LESSON_SEARCH_MOVES) dfs(applyMove(s, m), [...path, m], depth - 1)
    }
    for (let d = 1; d <= 5 && !example; d++) dfs(base, [], d)
    expect(example, 'expected some ≤5-move scramble to hit middle-layer side-connect').not.toBeNull()
    const student = applyMoves(base, example!)
    ...
    if (step.kind === 'side-connect') {
      ...
      expect(demoMove === 'F' || demoMove === "F'" || demoMove === 'R' || demoMove === "R'" || demoMove === 'L' || demoMove === "L'" || demoMove === 'B' || demoMove === "B'").toBe(
        true,
      )
    }
  })
  ...
  it('rotate-bottom demo uses D2 when two quarter D turns align the active slot', () => {
    const student = applyMoves(cubeStateToStudentFrame(createSolvedCubeState()), ['D', 'D'])
    const step = getWhiteCrossLessonStep(student)
  ...
  it('short direct solve for U-layer slot (R R U) uses U′ R2, not a longer multi-step path', () => {
    ...
    const student = applyMoves(cubeStateToStudentFrame(createSolvedCubeState()), [...moves])
    const step = getWhiteCrossLessonStep(student)
    ...
    if ('demoMoves' in step && step.demoMoves) {
      ...
      expect(compressConsecutiveFaceQuarterTurns(step.demoMoves)).toEqual(step.demoMoves)
    }
  })
  ...
  it('suggests a cross step when a white cross edge is not on the bottom', () => {
    ...
    expect(broken, 'at least one single move should break the white cross').toBeDefined()
    const step = getWhiteCrossLessonStep(broken!)
    expect(['solve-edge', 'rotate-bottom', 'side-connect', 'insert-double']).toContain(step.kind)
  })
  ...
  it('solve-edge step describes center alignment and slotting on the bottom', () => {
    ...
    if (step.kind === 'solve-edge') {
      ...
      expect(step.body.toLowerCase()).not.toMatch(/white sticker should face straight up on the yellow/)
    }
  })
  ...
  it('solve-edge example demos are verified slot solves for the partner edge', () => {
    const student = applyMoves(cubeStateToStudentFrame(createSolvedCubeState()), ['F', 'R', 'U'])
  ...
  it('permutes a ready cross edge before solve-edge on a mis-flipped top-layer edge on the same scramble', () => {
    const moves = "R D F R B' D' R' L B2 R2 R F B2 B L D2 L2 B2".split(/\s+/).filter(Boolean) as Move[]
    let student = cubeStateToStudentFrame(createSolvedCubeState())
    for (const m of moves) {
      student = applyMove(student, m as Move)
    }
    const step = getWhiteCrossLessonStep(student)
    expect(['side-connect', 'rotate-bottom', 'insert-double']).toContain(step.kind)
  ...
  it('permutes a ready edge before solve-edge when another cross edge still needs work (R R U then F)', () => {
    const withSlotReady = applyMoves(cubeStateToStudentFrame(createSolvedCubeState()), ['R', 'R', 'U'])
  ...
  it('fixture (F from solved): opening step is side-connect; applying its demo advances the lesson', () => {
    ...
    expect(['solve-edge', 'rotate-bottom', 'insert-double', 'complete']).toContain(step2.kind)
  })

  it('fixture (F from solved): solve-edge / insert-double / side-connect demos never reduce solved cross-slot count', () => {
    let storage = practiceFixtureCubeSingleF()
    let student = cubeStateToStudentFrame(storage)
    const guardedKinds = ['solve-edge', 'insert-double', 'side-connect'] as const
    for (let guard = 0; guard < 80; guard += 1) {
      ...
      if (guardedKinds.includes(step.kind as (typeof guardedKinds)[number])) {
        expect(countSolvedCrossSlots(student)).toBeGreaterThanOrEqual(beforeSlots)
      }
    }
  })
  ...
  it('fixture (F from solved): playable lesson steps through the cross walk', () => {
    ...
    const step = getWhiteCrossLessonStep(student)
    expect(['side-connect', 'solve-edge', 'rotate-bottom', 'insert-double']).toContain(step.kind)
    if ('demoMoves' in step && step.demoMoves?.length) {
      expect(step.demoMoves.length).toBeGreaterThan(0)
    }
  })
  ...
  it('simulateLesson with avoid-back on B-steps matches getLessonExecutionMoves per step', () => {
    ...
    while (steps < 80 && !isWhiteCrossComplete(student)) {
      ...
      const { moves } = getLessonExecutionMoves(step.demoMoves, useAvoid, noneHold())
