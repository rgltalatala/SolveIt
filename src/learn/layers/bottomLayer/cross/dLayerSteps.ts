import { applyMoves, compressConsecutiveFaceQuarterTurns, faceDoubleTurn } from '../../../../cube/cubeState'
...
import type { CrossEdgeId, DPhaseOption, PermuteReadyCandidate, WhiteCrossLessonStep } from './types'
...
function findMinimalBottomSpinToSolveSlot(studentState: CubeState, id: CrossEdgeId): Move[] | null {
...
function tryDPrefixOrInsertSolveSlot(studentState: CubeState, id: CrossEdgeId): Move[] | null {
...
function buildDLayerStepFromOption(studentState: CubeState, best: DPhaseOption): WhiteCrossLessonStep {
  ...
  if (best.variant === 'insert-double') {
    const faceTurn = best.demo.find((m) => m === 'F2' || m === 'R2' || m === 'L2' || m === 'B2')
...
export function collectRotateBottomPermuteCandidates(studentState: CubeState): PermuteReadyCandidate[] {
  ...
  for (const id of unsolvedWhiteOnDSlotIds(studentState)) {
    ...
    if (bottomSpinMoves && isVerifiedSlotDemo(studentState, id, bottomSpinMoves)) {
      candidates.push({
        id,
...
export function collectDLayerInsertPermuteCandidates(studentState: CubeState): PermuteReadyCandidate[] {
  const candidates: PermuteReadyCandidate[] = []
  for (const id of unsolvedWhiteOnDSlotIds(studentState)) {
    if (slotShowsRotateBottomPattern(studentState, id)) continue
    const insert = tryDPrefixOrInsertSolveSlot(studentState, id)
    if (insert && insert.length > 0 && isVerifiedSlotDemo(studentState, id, insert)) {
      candidates.push({
        id,
        step: buildDLayerStepFromOption(studentState, { id, demo: insert, variant: 'insert-double' }),
      })
    }
  }
  return candidates
...
export function collectDLayerPermuteCandidates(studentState: CubeState): PermuteReadyCandidate[] {
  return [
