...
import { applyLessonToStorage, getLessonExecutionMoves, noneHold } from './index'

describe('applyLessonToStorage', () => {
  it('returns null for empty demo', () => {
    expect(applyLessonToStorage(createSolvedCubeState(), [], noneHold(), false)).toBeNull()
  })
  ...
  it('matches manual getLessonExecutionMoves + applyMovesInStudentHold', () => {
    ...
    const expected = applyMovesInStudentHold(storage, getLessonExecutionMoves(raw, false).moves)
