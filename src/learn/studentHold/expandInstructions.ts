...
import type { AvoidBackPrefs, ExpandDemoResult, Instruction, StudentHold } from './types'
...
export function expandDemoToInstructions(
  ...
  const { steps, finalHold } = expandDemoSteps(rawMoves, initialHold, prefs.avoidBackMoves)
  ...
  for (const step of steps) {
    if (step.type === 'rotation') {
      instructions.push({
        ...
        text: getRotationText(step.rotation, step.purpose as RotationPurpose | undefined),
      })
