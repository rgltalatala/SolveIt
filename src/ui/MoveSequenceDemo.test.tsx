...
import { createSolvedCubeState, cubeStateToStudentFrame } from '../cube/cubeState'
...
vi.mock('../cube3d/CubeView', () => ({
  CubeView: ({
    ...
    <div data-testid="cube-view">
      {moveAnimation ? (
        <button type="button" data-testid="complete-animation" onClick={() => moveAnimation.onComplete()}>
...
describe('MoveSequenceDemo', () => {
  ...
  it('steps forward through moves with animation', async () => {
    ...
    render(<MoveSequenceDemo baseCubeState={baseCubeState} moves={['F', 'R']} />)
  ...
  it('animates reverse on Previous move', async () => {
    ...
    expect(screen.getByTestId('complete-animation')).toHaveTextContent('complete-F-reverse')
    await user.click(screen.getByTestId('complete-animation'))
    expect(screen.getByText('Start position')).toBeInTheDocument()
  })
