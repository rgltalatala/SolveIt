...
export function ScanView() {
  ...
  const assembledFromScanned = useMemo(() => cubeStateFromScannedFaces(scannedFaces), [scannedFaces])

  const inManualFix = !currentFace && validationIssues.length > 0 && assembledFromScanned !== null
  ...
  const handleCapture = () => {
    ...
    if (!frame) return
    setDetectedFace(detectorFaceToCubeJsFaceOrder(currentFace, detectFaceColorsFromImageData(frame)))
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4">
          {!isCorrecting || !detectedFace ? (
            <ScannerOverlay videoRef={videoRef} isReady={isReady} error={error} onCapture={handleCapture} />
          ) : (
            <CorrectionPanel
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm text-slate-400">
            Gray stickers are not scanned yet. Confirm each face to populate this cube in real time.
          </p>
