...
function partitionCubies(spec: MoveAnimationSpec | null) {
  if (!spec) {
    return { staticCubies: cubieDefinitions, layerCubies: [] as typeof cubieDefinitions }
  }
  if (spec.kind === 'whole') {
    return { staticCubies: [] as typeof cubieDefinitions, layerCubies: cubieDefinitions }
  }
  const layerCubies = cubieDefinitions.filter((c) => cubieOnFaceLayer(c.position, spec.face))
  const staticCubies = cubieDefinitions.filter((c) => !cubieOnFaceLayer(c.position, spec.face))
...
function CubieAt({
  ...
  return (
    <group position={[x, y, z]}>
      <Cubie position={[0, 0, 0]} faceColors={getCubieFaceColors(cubeState, position)} />
    </group>
  )
}
 ...
 */
export function AnimatedCubeMesh({ cubeState, animation }: AnimatedCubeMeshProps) {
  const spec = animation ? getMoveAnimationSpec(animation.move) : null
  const { staticCubies, layerCubies } = useMemo(() => partitionCubies(spec), [spec, animation?.move])
  ...
  useFrame((_, delta) => {
    ...
    if (!animation || !activeSpec || !turnRef.current || completedRef.current) return

    progressRef.current = Math.min(1, progressRef.current + delta / (MOVE_ANIMATION_MS / 1000))
  ...
  return (
    <>
      {staticCubies.map((c) => (
        <CubieAt key={c.position.join(':')} cubeState={cubeState} position={c.position} />
      ))}
      <group ref={turnRef}>
        {layerCubies.map((c) => (
          <CubieAt key={c.position.join(':')} cubeState={cubeState} position={c.position} />
