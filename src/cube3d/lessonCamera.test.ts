...
describe('lessonCamera', () => {
  it('snaps to opposite azimuth from baseline, not cumulative user offset', () => {
    ...
    const result = snapLessonCamera(controls as never, baseline, 'lessonHoldOpposite')
    expect(result).toBeCloseTo(baseline.azimuth + Math.PI, 5)
    expect(controls.setAzimuthalAngle).toHaveBeenCalledWith(baseline.azimuth + Math.PI)
