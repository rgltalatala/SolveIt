...
export function captureLessonCamera(controls: OrbitControlsImpl): LessonCameraSnapshot {
  return {
...
export function snapLessonCamera(
  ...
  const azimuth = view === 'lessonHoldOpposite' ? snapshot.azimuth + Math.PI : snapshot.azimuth
