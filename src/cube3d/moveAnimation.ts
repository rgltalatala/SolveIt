...
import { getFaceFromMove, getModifierFromMove } from '../learn/studentHold/translateMove'
...
export function cubieOnFaceLayer([x, y, z]: CubiePosition, face: Face): boolean {
  switch (face) {
    case 'U':
      return y === 1
    case 'D':
...
export function getMoveAnimationSpec(move: Move): MoveAnimationSpec {
  if (isWholeCubeRotation(move)) {
    ...
    const modifier: '' | "'" | '2' = move.includes('2') ? '2' : move.includes("'") ? "'" : ''
