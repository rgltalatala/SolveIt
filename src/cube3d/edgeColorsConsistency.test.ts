...
import { applyMoves, createSolvedCubeState, cubeStateToStudentFrame } from '../cube/cubeState'
import { cubeJsStickerSlots, cubieDefinitions, faceStickerIndex, getCubieFaceColors } from './cubeGeometry'

function assertEdgesDistinctColors(label: string, s: import('../cube/cubeState').CubeState) {
...
describe('edge sticker indexing & scrambled cubes', () => {
  it('solved cube after turnX2 has distinct colors on every edge', () => {
    assertEdgesDistinctColors('student solved', cubeStateToStudentFrame(createSolvedCubeState()))
  })
  ...
  it('every edge slot resolves to a non-center fallback index in cubeJsStickerSlots', () => {
    for (const { position, exposedFaces } of cubieDefinitions) {
      ...
      for (const face of exposedFaces) {
        ...
        const found = slots.some(([sx, sy, sz]) => sx === position[0] && sy === position[1] && sz === position[2])
        expect(found, `position ${position.join()} must appear on face ${face}`).toBe(true)
        expect(idx, `fallback index 4 only for face-center cubie on ${face}`).not.toBe(4)
      }
    }
  })
