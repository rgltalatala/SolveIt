...
import { getLessonExecutionMoves, isBackFaceMove, noneHold } from '../learn/studentHold'
...
describe('cubeStore lesson session', () => {
  ...
  it('loadScrambledCubeIntoLesson resets lesson session', () => {
    useCubeStore.setState({ studentHold: { y: 'y2' }, hasSeenAvoidBackCallout: true })
  ...
  it('applyLessonDemoMoves delegates to applyLessonStep without avoid-back', () => {
    ...
    expect(useCubeStore.getState().cubeState).toEqual(applyMovesInStudentHold(before, raw))
  })
  ...
  it('resetLessonSession clears hold and callout flag', () => {
    useCubeStore.setState({ studentHold: { y: 'y' }, hasSeenAvoidBackCallout: true })
