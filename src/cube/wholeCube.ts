...
import { cubeJsStringToCubeState, cubeStateToCubeJsString } from './cubeStateToFacelets'
...
export function wholeCubeMove(state: CubeState, notation: string): CubeState {
  const c = Cube.fromString(cubeStateToCubeJsString(state)) as unknown as CubeJsMutable
  c.move(notation)
