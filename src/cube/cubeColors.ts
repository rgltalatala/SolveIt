...
export const colorRanges: Record<Color, ColorRangeConfig> = {
  white: { primary: { hMin: 0, hMax: 360, sMin: 0, sMax: 30, vMin: 70, vMax: 100 } },
  yellow: { primary: { hMin: 45, hMax: 75, sMin: 50, sMax: 100, vMin: 50, vMax: 100 } },
  green: { primary: { hMin: 90, hMax: 150, sMin: 40, sMax: 100, vMin: 30, vMax: 100 } },
  blue: { primary: { hMin: 200, hMax: 240, sMin: 40, sMax: 100, vMin: 30, vMax: 100 } },
  red: {
    primary: { hMin: 0, hMax: 15, sMin: 60, sMax: 100, vMin: 40, vMax: 100 },
    wrap: { hMin: 345, hMax: 360, sMin: 60, sMax: 100, vMin: 40, vMax: 100 },
  },
  orange: { primary: { hMin: 15, hMax: 45, sMin: 60, sMax: 100, vMin: 40, vMax: 100 } },
}
...
export function classifyHsvToColor(hsv: Hsv): Color {
  for (const [color, config] of Object.entries(colorRanges) as [Color, ColorRangeConfig][]) {
    if (inRange(hsv, config.primary) || (config.wrap && inRange(hsv, config.wrap))) {
      return color
    }
  }
  ...
  for (const [color, prototype] of Object.entries(colorPrototypes) as [Color, Hsv][]) {
