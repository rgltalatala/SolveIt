...
import { applyMove, createSolvedCubeState, FACE_COLOR_CONVENTION } from './cubeState'
...
function cubeJsFaceletsToState(facelets: string): CubeState {
  const letterToColor = (letter: string): Color => FACE_COLOR_CONVENTION[letter as Face]
  let offset = 0
...
describe('validateCubeState', () => {
  ...
  it('fails when a color count is not exactly nine', () => {
    ...
    expect(result.issues.some((issue) => issue.kind === 'color-count' && issue.color === 'red')).toBe(true)
  })
  ...
  it('fails when a face center does not match face identity', () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.issues.some((issue) => issue.kind === 'center-mismatch' && issue.face === 'U')).toBe(true)
  })
  ...
  it('fails when the cube is unsolvable even with valid counts and centers', () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.issues.some((issue) => issue.kind === 'unsolvable')).toBe(true)
  })

  it('uses the short duplicate-corner message when corner facelets are inconsistent', () => {
    const facelets =
      'UULDUFBDRBBRBRLDDLFRRLFLLBRBRFDDLFRBFUDBLFUFUDRUUBUDFL'
    const state = cubeJsFaceletsToState(facelets)
    const result = validateCubeState(state)
    expect(result.valid).toBe(false)
    const msg = result.issues.find((i) => i.kind === 'unsolvable')?.message ?? ''
    expect(msg).toBe(
      'Corner pieces do not line up with a real cube.',
    )
  })
})
