...
import { COLOR_TO_FACE, COLORS, FACE_COLOR_CONVENTION, FACE_ORDER } from './cubeState'
...
function findSuggestedFace(issues: CubeValidationIssue[]): Face | null {
  const deficitColorIssue = issues.find(
    (issue) => issue.kind === 'color-count' && issue.color && typeof issue.count === 'number' && issue.count < 9,
  )
...
export function validateCubeStateBasic(state: CubeState): CubeValidationResult {
  ...
  if (centers.size !== 6) {
    issues.push({
      ...
      message: 'Face centers must all be unique colors. Please rescan the face with the wrong center color.',
    })
  }
...
function validateCubeStateSolvable(state: CubeState): CubeValidationIssue[] {
  try {
    ...
    const cpOk =
      cubeJson.cp.length === 8 && cubeJson.cp.every((v) => Number.isInteger(v) && v >= 0 && v < 8)
    const epOk =
      cubeJson.ep.length === 12 && cubeJson.ep.every((v) => Number.isInteger(v) && v >= 0 && v < 12)
    if (!cpOk || !epOk) {
    ...
    const cornerOrientationValid = cubeJson.co.reduce((sum, value) => sum + value, 0) % 3 === 0
    if (!cornerOrientationValid) {
      return [{ kind: 'unsolvable', message: messages.cornerTwist }]
    }

    const edgeOrientationValid = cubeJson.eo.reduce((sum, value) => sum + value, 0) % 2 === 0
    ...
    return [{ kind: 'unsolvable', message: `${messages.parseError} (${detail})` }]
  }
}
