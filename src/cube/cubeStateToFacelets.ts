...
export function cubeStateToFaceletString(cubeState: CubeState, order: Face[] = FACELET_ORDER): string {
  return order
    .map((face) => cubeState[face].map((color) => COLOR_TO_FACE[color]).join(''))
    .join('')
}
